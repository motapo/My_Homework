...
// -----------------массив имен с сортировкой по кол-ву друзей -----------
//тянем из общего массива значения Имени и Друзей с помощью "_.map"
var ArrName = _.map(obj, 'name');
var ArrFriends = _.map(obj, 'friends');

//мостим два новых массива в один как объекты
var ArrNameFriends = [];
for (var i = ArrName.length - 1; i >= 0; i--) {
  //будет два поля - 1.Имя и 2.Kол-во друзей, оно же равное длинне объекта с друзьями в obj 
  ArrNameFriends[i] = { 'name': ArrName[i], 'friends' : ArrFriends[i].length};
}
console.log('All Name Friends: ', ArrNameFriends);

//сортируем по количеству друзей с помощью "_.sortBy"
var SortNames = _.sortBy(ArrNameFriends, 'friends');
console.log('Sort Names: ', SortNames);
console.log('--------------------------------------------------');
// -----------------массив Skills с алфавитной сортировкой -----------

// создаем массив из объектов skills массива obj
var SkillsAll = _.map(obj, 'skills');
console.log('Arr Skills All: ', SkillsAll);

//вытягиваем в один массив все значения Skills 
var flatSkills = _.flatten(SkillsAll);

//убираем дубликаты
var uniqSkills = _.uniq(flatSkills);

//сортируем полученнный массив 
var sortResult = _.sortBy(uniqSkills);
console.log('sortResult: ', sortResult);
console.log('--------------------------------------------------');

// -----------------массив Name Frends Uniqum с алфавитной сортировкой -----------

// вытягиваем все наборы 'friends' из obj
var allFriends = _.map(obj, 'friends');
console.log('Friends + Id: ', allFriends);

//вытягиваем в один массив объекты 'friends + id'  
var flatFriends = _.flatten(allFriends);
console.log('Arr "Friends, Id": ',flatFriends);

//вытягиваем из flatFriends значения 'name'
var allFriendsName = _.map(flatFriends, 'name');
console.log('Arr Friends Name: ', allFriendsName);

//оставляем в массиве только уникальные имена, потворяющиеся удаляются
var uniqFrendsName = _.uniq(allFriendsName);
console.log('Uniqum Name Frands: ', uniqFrendsName);
console.log('--------------------------------------------------');
